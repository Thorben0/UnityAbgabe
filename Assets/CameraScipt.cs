...
public class CameraScipt : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        transform.position = Vector3.Lerp(transform.position, target.transform.position + offset, slerp);

        /* Now update the cameras rotation */
        transform.rotation = Quaternion.Slerp(transform.rotation, Quaternion.LookRotation(lookVec), slerp);
    }
}
